...
class Voyager:
    def __init__(
        ...
        # set openai api key & google api key
    ...
    def reset(self, task, context="", reset_env=True, init_inventory={}, init_code = ""):
        ...
        human_message = self.action_agent.render_human_message(
            events=events, code=init_code, task=self.task, context=context, critique=""
        )
    ...
    def step(self, init_message = ""):
        if self.action_agent_rollout_num_iter < 0:
            raise ValueError("Agent must be reset before stepping")
        ai_message = self.action_agent.llm.chat(self.messages)
    ...
    def rollout(self, *, task, context, reset_env=True, init_inventory={}, init_message=None):
        self.reset(task=task, context=context, reset_env=reset_env, init_inventory=init_inventory, init_code=init_message) # init code
        while True:
