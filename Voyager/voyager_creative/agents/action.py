...
from .llm_wrapper import ChatWrapper
from langchain.prompts import SystemMessagePromptTemplate
from langchain.schema import AIMessage, HumanMessage, SystemMessage

from voyager_creative.prompts import load_prompt
from voyager_creative.control_primitives_context import load_control_primitives_context
...
class ActionAgent:
    ...
    def update_chest_memory(self, chests):
        """
        Update surrounding chest information
        """
    ...
    def render_chest_observation(self):
        """
        Get surrounding chest information
        """
        chests = []
    ...
    def render_system_message(self, skills=[]):
        ...
        system_message_prompt = SystemMessagePromptTemplate.from_template( # lang chain prompts
            system_template
        )
    ...
    def process_ai_message(self, message):
        ...
        while retry > 0:
            try:
                ...
                assert code, "No JavaScript code found in the message."
                ...
                assert len(functions) > 0, "No function declarations were found in the code."
                ...
                for function in reversed(functions):
                    if function["type"] == "AsyncFunctionDeclaration":
                        ...
                        break
                # 修正を入れる箇所 (空リストの場合のチェック)
                assert main_function is not None, "No async function found. Your main function must be async."
                assert len(main_function["params"]) == 1, f"Main function {main_function['name']} must take a single argument."
                assert main_function["params"][0].name == "bot", f"Main function {main_function['name']} must take a single argument named 'bot'"
